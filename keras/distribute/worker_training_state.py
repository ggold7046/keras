...
class WorkerTrainingState:
    ...
    def backup_if_preempted(self):
        if self._enable_save_before_preemption:
            # This function call will check preemption signal. For
            # MultiWorkerMirroredStrategy, it'll also save checkpoint if a
            # signal is available. For TPUStrategy, it will signal
            # DataHandler.catch_stop_iteration to save a checkpoint.
            self.preemption_handler.run(lambda: None)
